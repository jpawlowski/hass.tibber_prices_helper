...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `TibberPricesHelper`
...
- Inherit from both platform entity and `TibberPricesHelperEntity` (order matters)
...
from custom_components.tibber_prices_helper.sensor import async_setup_entry
...
**Config entry data:** `entry_data: TibberPricesHelperData = hass.data[DOMAIN][entry.entry_id]`
