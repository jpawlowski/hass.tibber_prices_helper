...
This document describes all configuration options and settings available in the Tibber Price Information Integration Blueprint Ratings Helper custom integration.
...
### `tibber_prices_helper.example_service`
...
```yaml
service: tibber_prices_helper.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: tibber_prices_helper.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Tibber Price Information Integration Blueprint Ratings Helper Alert
...
- [GitHub Issues](https://github.com/jpawlowski/tibber_prices_helper/issues) - Report problems
