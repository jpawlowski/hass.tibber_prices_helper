...
This document describes the technical architecture of the Tibber Price Information Integration Blueprint Ratings Helper custom component for Home Assistant.

## Directory Structure

```text
custom_components/tibber_prices_helper/
...
│   ├── __init__.py          # Exports TibberPricesHelperDataUpdateCoordinator
...
│   ├── __init__.py          # Exports TibberPricesHelperEntity
...
- `base.py` - Main coordinator class (`TibberPricesHelperDataUpdateCoordinator`)
...
**Key class:** `TibberPricesHelperDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `TibberPricesHelperApiClient`
...
- `TibberPricesHelperConfigFlowHandler` (main flow)
- `TibberPricesHelperOptionsFlow` (options)
...
**Key class:** `TibberPricesHelperEntity` (in `entity/base.py`)
...
2. `TibberPricesHelperEntity` for common functionality
...
1. Create directory: `custom_components/tibber_prices_helper/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `TibberPricesHelperEntity`
