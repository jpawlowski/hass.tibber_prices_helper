...
- **Repository:** jpawlowski/tibber_prices_helper

**Key directories:**

- `custom_components/tibber_prices_helper/` - Main integration code
...
- Integration logs: `custom_components.tibber_prices_helper: debug` in `config/configuration.yaml`
...
- Use the domain `tibber_prices_helper` for all DOMAIN references
- Prefix all integration-specific classes with `TibberPricesHelper`
- Use "Tibber Price Information Integration Blueprint Ratings Helper" as the display title
...
- Inherit from platform base + `TibberPricesHelperEntity`
...
- `tests/` mirrors `custom_components/tibber_prices_helper/` structure
